...
A **"Workflow"** is an optional way to _combine multiple tasks together_ in a way that can be gracefully retried from the point of failure.
...
### [Why use Workflows?](/docs/jobs-queue/workflows#why-use-workflows)#### [Single Task vs Workflow](/docs/jobs-queue/workflows#single-task-vs-workflow)
...
### [Defining a workflow](/docs/jobs-queue/workflows#defining-a-workflow)
...
`concurrency` |  Control how jobs with the same concurrency key are handled. Jobs with the same key will run exclusively (one at a time). Requires `jobs.enableConcurrencyControl: true` to be set. See Concurrency Controls below for details.   
...
#### [Running tasks inline](/docs/jobs-queue/workflows#running-tasks-inline)
...
### [Understanding Workflow failure & recovery](/docs/jobs-queue/workflows#understanding-workflow-failure-and-recovery)
...
#### [Scenario: Email service is down](/docs/jobs-queue/workflows#scenario-email-service-is-down)
...
The entire handler function re-runs, but completed tasks return their cached results immediately without re-executing their logic.

### [Accessing Task outputs](/docs/jobs-queue/workflows#accessing-task-outputs)
...
### [Workflow best practices](/docs/jobs-queue/workflows#workflow-best-practices)#### [Use descriptive task IDs](/docs/jobs-queue/workflows#use-descriptive-task-ids)
```
// Hard to debug
...
#### [Keep tasks small and focused](/docs/jobs-queue/workflows#keep-tasks-small-and-focused)
```
// Task does too much
    {
      slug: 'onboardUser',
...
#### [Pass IDs, not entire objects](/docs/jobs-queue/workflows#pass-ids-not-entire-objects)
```
...
#### [Set appropriate retry counts](/docs/jobs-queue/workflows#set-appropriate-retry-counts)
...
#### [Handle errors with context](/docs/jobs-queue/workflows#handle-errors-with-context)
```
...
### [Concurrency Controls](/docs/jobs-queue/workflows#concurrency-controls)
...
#### [Enabling Concurrency Controls](/docs/jobs-queue/workflows#enabling-concurrency-controls)
...
#### [How It Works](/docs/jobs-queue/workflows#how-it-works)
...
When you define a `concurrency` key:
  ...
  * If multiple pending jobs with the same key are picked up in the same batch, only the first one (by creation order) runs - the others are released back to `processing: false` and will be picked up on subsequent runs
  ...
  1. **Result:** Jobs with the same concurrency key are guaranteed to run sequentially, never in parallel. All jobs are preserved and will eventually complete - they just wait their turn.

#### [Concurrency Configuration Options](/docs/jobs-queue/workflows#concurrency-configuration-options)
...
#### [Common Patterns](/docs/jobs-queue/workflows#common-patterns)
...
#### [Supersedes Behavior](/docs/jobs-queue/workflows#supersedes-behavior)
...
#### [Important Considerations](/docs/jobs-queue/workflows#important-considerations)
...
[Next Jobs](/docs/jobs-queue/jobs)
