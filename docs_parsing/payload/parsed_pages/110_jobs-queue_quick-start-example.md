<a id="page-110"></a>
---
url: https://payloadcms.com/docs/jobs-queue/quick-start-example
---
...
### [Step 1: Define a Task](/docs/jobs-queue/quick-start-example#step-1-define-a-task)
...
### [Step 2: Queue the Job trigger](/docs/jobs-queue/quick-start-example#step-2-queue-the-job-trigger)
```
{
      slug: 'users',
      hooks: {
...
### [Step 3: Run the Jobs](/docs/jobs-queue/quick-start-example#step-3-run-the-jobs)
```
export default buildConfig({
      // ... other config
      jobs: {
...
The [`autoRun`](../jobs-queue/queues#cron-jobs) configuration automatically processes queued jobs on a schedule using cron syntax. In this example, Payload checks for pending jobs every 5 minutes and executes them. Alternatively, you can [manually trigger job processing](../jobs-queue/queues#local-api) with `payload.jobs.run()` or use the [API endpoint](../jobs-queue/queues#endpoint) for serverless platforms.
...
## [Example 2: Recurring Scheduled Job](/docs/jobs-queue/quick-start-example#example-2-recurring-scheduled-job)
...
### [Why Use Scheduled Jobs?](/docs/jobs-queue/quick-start-example#why-use-scheduled-jobs)
...
### [Step 1: Define a Task with Schedule](/docs/jobs-queue/quick-start-example#step-1-define-a-task-with-schedule)
```
import { buildConfig } from 'payload'
...
### [Step 2: Configure the Job Runner](/docs/jobs-queue/quick-start-example#step-2-configure-the-job-runner)
...
### [How It Works](/docs/jobs-queue/quick-start-example#how-it-works)
...
**Critical:** Both `schedule.queue` and `autoRun.queue` must use the same queue name ('reports' in this example). If they don't match, jobs will be queued but never executed.

### [Complete Configuration](/docs/jobs-queue/quick-start-example#complete-configuration)
...
**For Serverless Platforms:** If deploying to Vercel or similar platforms, `autoRun` won't work. Use the [Vercel Cron approach](../jobs-queue/queues#vercel-cron-example) instead.

### [When to Use Each Approach](/docs/jobs-queue/quick-start-example#when-to-use-each-approach)
...
See [Job Schedules](../jobs-queue/schedules) for more details on scheduling options and advanced patterns.

[Next Tasks](/docs/jobs-queue/tasks)
