...
## [Local API](/docs/queries/select#local-api)
...
### [Empty select](/docs/queries/select#empty-select)
...
## [REST API](/docs/queries/select#rest-api)
...
## [defaultPopulate collection config property](/docs/queries/select#defaultpopulate-collection-config-property)
...
![How to easily optimize Payload CMS requests with defaultPopulate](https://payloadcms.com/_next/image?url=https%3A%2F%2Fimg.youtube.com%2Fvi%2FSnqjng_w-QU%2Fmqdefault.jpg&w=3840&q=75)

**Watch default populate in action**

**Important:** When using `defaultPopulate` on a collection with [Uploads](../fields/upload) enabled and you want to select the `url` field, it is important to specify `filename: true` as well, otherwise Payload will not be able to construct the correct file URL, instead returning `url: null`.

## [Populate](/docs/queries/select#populate)
