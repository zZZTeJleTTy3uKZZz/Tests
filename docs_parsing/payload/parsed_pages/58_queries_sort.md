...
Because sorting is handled by the database, the field cannot be a [Virtual Field](/blog/learn-how-virtual-fields-can-help-solve-common-cms-challenges) unless it's [linked with a relationship field](../fields/relationship#linking-virtual-fields-with-relationships). It must be stored in the database to be searchable.

**Tip:** For performance reasons, it is recommended to enable `index: true` for the fields that will be sorted upon. [More details](../database/indexes).

## [Local API](/docs/queries/sort#local-api)
...
## [REST API](/docs/queries/sort#rest-api)
...
import { stringify } from 'qs-esm'
    ...
    const getPosts = async () => {
      ...
      const response = await fetch(
        `https://localhost:3000/api/posts${stringifiedQuery}`, 
      )
...
## [GraphQL API](/docs/queries/sort#graphql-api)
...
[Next Select](/docs/queries/select)
