...
![Drafts Enabled](https://payloadcms.com/images/docs/autosave-drafts.jpg)
...
## [ Options](/docs/versions/drafts#options)
...
`schedulePublish` |  Allow for editors to schedule publish / unpublish events in the future. More  
`validate` |  Set `validate` to `true` to validate draft documents when saved. Default is `false`.   
  
## [Database changes](/docs/versions/drafts#database-changes)
...
## [Draft API](/docs/versions/drafts#draft-api)
...
#### [Using the `draft` parameter](/docs/versions/drafts#using-the-draft-parameter)
...
#### [Reading drafts vs. published documents](/docs/versions/drafts#reading-drafts-vs-published-documents)
...
**Example scenario:**
  ...
  2. You update with `draft: true` to make changes without affecting the published version
  3. You update again with `draft: true` to make more draft changes
...
## [Controlling who can see Collection drafts](/docs/versions/drafts#controlling-who-can-see-collection-drafts)
...
#### [Restricting draft access](/docs/versions/drafts#restricting-draft-access)
...
## [Scheduled publish](/docs/versions/drafts#scheduled-publish)
...
**Important:** if you are going to enable scheduled publish / unpublish, you need to make sure your Payload app is set up to process [Jobs](../jobs-queue/overview). This feature works by creating a Job in the background, which will be picked up after the job becomes available. If you do not have any mechanism in place to run jobs, your scheduled publish / unpublish jobs will never be executed.

## [Unpublishing drafts](/docs/versions/drafts#unpublishing-drafts)
...
## [Reverting to published](/docs/versions/drafts#reverting-to-published)
...
[Next Autosave](/docs/versions/autosave)
