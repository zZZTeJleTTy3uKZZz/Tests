...
There are many Official Plugins available that solve for some of the most common use cases, such as the [Form Builder Plugin](./form-builder) or [SEO Plugin](./seo). There are also Community Plugins available, maintained entirely by contributing members. To extend Payload's functionality in some other way, you can easily [build your own plugin](./build-your-own).
...
Because we rely on a simple config-based structure, Payload Plugins simply take in an existing config and return a _modified_ config with new fields, hooks, collections, admin views, or anything else you can think of.
...
## [Official Plugins](/docs/plugins/overview#official-plugins)
...
You can also [build your own plugin](./build-your-own) to easily extend Payload's functionality in some other way. Once your plugin is ready, consider sharing it with the community.
...
## [Community Plugins](/docs/plugins/overview#community-plugins)
...
Some plugins have become so widely used that they are adopted as an Official Plugin, such as the [Lexical Plugin](https://github.com/AlessioGr/payload-plugin-lexical). If you have a plugin that you think should be an Official Plugin, please feel free to start a new [Discussion](https://github.com/payloadcms/payload/discussions).
...
## [Example](/docs/plugins/overview#example)
...
**Reminder:** See [how to build your own plugin](./build-your-own) for a more in-depth explication on how to create your own Payload Plugin.

[Next Building Your Own Plugin](/docs/plugins/build-your-own)
